...
export class PageSetupDialog {
   ...
   *
   */
  static addPageFormatPanel(div, namePostfix, pageFormat, _pageFormatListener) {
    ...
    mxEvent.addListener(portraitSpan, "click", (evt) => {
      portraitCheckBox.checked = true;
      update(evt);
      mxEvent.consume(evt);
    });

    mxEvent.addListener(landscapeSpan, "click", (evt) => {
    ...
    mxEvent.addListener(paperSizeSelect, "change", (evt) => {
      // Handles special case where custom was chosen
      this.customSize = paperSizeSelect.value == "custom";
      update(evt, true);
    });
  ...
  static update(_evt?, selectChanged?) {
    const {
      formatDiv,
      customDiv,
      widthInput,
  ...
  static listener(_sender?, _evt?, force?) {
    const {
      widthInput,
      heightInput,
      formats,
